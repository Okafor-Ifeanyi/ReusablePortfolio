...
function ExperienceForm({
  ...
  const boundAction = exp ? updateExperience.bind(null, exp.id) : createExperience
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-5 border border-gray-200 rounded-xl bg-white">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="date" name="startDate" required defaultValue={toDateInput(exp?.startDate ?? null)} className={input} />
        </div>
        <div>
      ...
      <div className="flex items-center gap-2">
        <input type="checkbox" id={`current-${exp?.id ?? 'new'}`} name="isCurrent" defaultChecked={exp?.isCurrent ?? false} className="w-4 h-4 rounded border-gray-300 accent-gray-900" />
        <label htmlFor={`current-${exp?.id ?? 'new'}`} className="text-sm text-gray-700">Current role</label>
      </div>

      <div className="flex items-center gap-3 pt-1">
        <SubmitButton pending={isPending}>{exp ? 'Save changes' : 'Add experience'}</SubmitButton>
        {onCancel && (
...
export default function ExperienceSection({ experiences }: { experiences: Experience[] }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {experiences.map((exp) =>
        editingId === exp.id ? (
          <ExperienceForm key={exp.id} exp={exp} onCancel={() => setEditingId(null)} />
        ) : (
          <div key={exp.id} className="flex items-start justify-between p-5 border border-gray-200 rounded-xl bg-white">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">
                {toDateInput(exp.startDate)} — {exp.isCurrent ? 'Present' : (toDateInput(exp.endDate) || '—')}
              </p>
              {exp.description && <p className="text-sm text-gray-600 mt-2">{exp.description}</p>}
            </div>
            <div className="flex items-center gap-3 shrink-0 ml-4">
              <button onClick={() => setEditingId(exp.id)} className="text-sm text-gray-500 hover:text-gray-900">
                Edit
              </button>
      ...
      {adding ? (
        <ExperienceForm onCancel={() => setAdding(false)} />
      ) : (
        <button
