  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-5 border border-gray-200 rounded-xl bg-white">
            ...
            {availableSkills.map((skill) => (
              <label key={skill.id} className="flex items-center gap-1.5 cursor-pointer">
                <input
                  ...
                  name="skillIds"
                  value={skill.id}
                  defaultChecked={linkedSkillIds.has(skill.id)}
      ...
      <div className="flex items-center gap-2">
        <input type="checkbox" id={`featured-${project?.id ?? 'new'}`} name="featured" defaultChecked={project?.featured ?? false} className="w-4 h-4 rounded border-gray-300 accent-gray-900" />
        <label htmlFor={`featured-${project?.id ?? 'new'}`} className="text-sm text-gray-700">Feature this project</label>
      </div>

      <div className="flex items-center gap-3 pt-1">
        <SubmitButton pending={isPending}>{project ? 'Save changes' : 'Add project'}</SubmitButton>
        {onCancel && (
          ...
          <div key={project.id} className="flex items-start justify-between p-5 border border-gray-200 rounded-xl bg-white">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {project.projectType && (
                  <span className="text-xs px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded-full">{project.projectType}</span>
                )}
                {project.featured && (
                  <span className="text-xs px-1.5 py-0.5 bg-amber-50 text-amber-600 rounded-full font-medium">Featured</span>
                )}
              </div>
              ...
              {project.projectSkills.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-2">
                  {project.projectSkills.map((ps) => (
                    <span key={ps.skillId} className="text-xs px-2 py-0.5 bg-gray-50 border border-gray-200 rounded-full text-gray-600">
                      {ps.skill.name}
                    </span>
                  ))}
                </div>
